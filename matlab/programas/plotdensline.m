function plotdensline(time,direction,pos,factor)
    ...
    if exist(currfile,'file')
        ...
        if (currtime >= time)
            ...
            if (max(size(intgrprmtrs.value)) < 70) % Grilla uniforme
                ...
                dum = h5read(currfile,'/dens');
                if (dir1 == 1); dens(dimarrays:dimarrays+ncll2-1) = dum(idxcll,:,1);
                elseif (dir1 == 2); dens(dimarrays:dimarrays+ncll2-1) = dum(:,idxcll,1);
                end
                ...
                for nb = 1:nblocks-1
                    if ((reflevel(nb) >= reflevel(nb+1))  && ...
                        ...
                        dum = h5read(currfile,'/dens');
                        if (dir1 == 1); dens(dimarrays:dimarrays+ncll2-1) = dum(idxcll,:,1,idxblk);
                        elseif (dir1 == 2); dens(dimarrays:dimarrays+ncll2-1) = dum(:,idxcll,1,idxblk);
                        end
                        dimarrays = dimarrays + ncll2;
                    end
                end
            ...
            dens = dens(order);
            imax = max(size(coords)); % Para dibujar la mitad del dominio en y
            if dir1 == 2;
                imin = ceil(imax/2);
            else
                imin = 1;
            end
            plot(coords(imin:imax),factor*dens(imin:imax),'.-k'); hold on
            %semilogy(coords(imin:imax),factor*dens(imin:imax),'.-k'); hold on
